...
def collect_floor_textures(path_to_floor_plan_textures):
    ...
    if not floor_textures:
        raise RuntimeError(
            f"No floor texture images found under {path_to_floor_plan_textures}"
        )
...
def get_floor_plan(scene, floor_textures):
    ...
    floor = TexturedMesh.from_faces(
        vertices=vertices, uv=uv, faces=faces, material=textured_material(texture)
    )
...
def render(scene, renderables, color, mode, frame_path=None):
    ...
    scene.clear()
    for r, c in zip(renderables, color, strict=False):
...
def scene_from_args(args):
    ...
    scene.camera_matrix = Matrix44.orthogonal_projection(
        left=-args.room_side,
        right=args.room_side,
        bottom=args.room_side,
        top=-args.room_side,
        near=0.1,
        far=6,
    )
...
def export_scene(output_directory, trimesh_meshes, names=None):
    if names is None:
        names = [f"object_{i:03d}.obj" for i in range(len(trimesh_meshes))]
    mtl_names = [f"material_{i:03d}" for i in range(len(trimesh_meshes))]
    ...
    for i, m in enumerate(trimesh_meshes):
        ...
        with open(path_to_mtl_file, "wb") as f:
            f.write(
                tex_out[mtl_key].replace(b"material0", mtl_names[i].encode("ascii"))
            )
...
def print_predicted_labels(dataset, boxes):
    ...
    print(f"The predicted scene contains {labels}")


def poll_specific_class(dataset):
    label = input(f"Select an object class from {dataset.object_types}\n")
