...
def poll_objects(dataset, current_boxes, scene_id):
    ...
    print(f"You are keeping the following indices {idxs_kept}")

    bbox_bounds = [
        float(ti) for ti in input("Enter bbox dims to place an object\n").split(",")
    ]
...
def render_to_folder(
    ...
    bbox_params_t = torch.cat(
        [boxes["class_labels"], boxes["translations"], boxes["sizes"], boxes["angles"]],
        dim=-1,
    ).cpu()
...
def sample_in_bbox(class_probs, translation_probs, bbox, trials=1000):
    ...
    def sample_dmll(probs, mu, s):
        ...
        u = np.random.rand()
        return np.clip(mu[i] + s[i] * (np.log(u) - np.log(1 - u)), -1, 1)
...
def main(argv):
    parser = argparse.ArgumentParser(
        description=(
            "Suggest a new object to be added based on a user "
    ...
    for i in range(args.n_sequences):
        ...
        render_to_folder(
            args,
            f"input_{current_scene.scene_id}_{i:03}",
            dataset,
            objects_dataset,
            tr_floor,
            floor_plan,
        ...
        with torch.no_grad():
            class_probs = network.distribution_classes(boxes=boxes, room_mask=room_mask)
            translation_probs = [
                network.distribution_translations(boxes, room_mask, c)
                for c in range(len(dataset.object_types))
            ]
        try:
            new_class, (tx, ty, tz) = sample_in_bbox(
                class_probs, translation_probs, bbox_bounds, args.trials
            )
        except RuntimeError:
            continue
        print(
            f"Adding {dataset.object_types[new_class]} at location: ({tx:.4f}, {ty:.4f}, {tz:.4f})"
        )
        ...
        path_to_objs = os.path.join(
            args.output_directory, f"suggestion_{current_scene.scene_id}_{i:03d}"
        )
        render_scene_from_bbox_params(
            args,
            bbox_params,
