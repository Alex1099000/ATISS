...
def main(argv):
    ...
    parser.add_argument(
        "output_directory", default="/tmp/", help="Path to output directory"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="configs/splits/invalid_threed_front_rooms.txt",
        help="Path to invalid scenes",
    )
    parser.add_argument(
        "--path_to_invalid_bbox_jids",
        default="configs/splits/black_list.txt",
    ...
    print(f"Loading dataset with {len(dataset)} rooms")
    ...
    print(
        f"Saving training statistics for dataset with bounds: {dataset.bounds} to {path_to_json}"
    )
    ...
    for (i, es), ss in tqdm(zip(enumerate(encoded_dataset), dataset, strict=False)):
        ...
        with DirLock(room_directory + ".lock") as lock:
            ...
            path_to_image = f"{room_directory}/rendered_scene_{args.window_size[0]}.png"
