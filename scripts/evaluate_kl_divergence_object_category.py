...
def main(argv):
    parser = argparse.ArgumentParser(
        description=(
            "Compute the KL-divergence between the object category "
    ...
    parser.add_argument(
        "--path_to_bounds", default=None, help="Path to the dataset's bounds"
    )
    parser.add_argument(
    ...
    network, _, validate_on_batch = build_network(
        ground_truth_scenes.feature_size,
        ground_truth_scenes.n_classes,
        config,
        args.weight_file,
        device=device,
    )
    ...
    gt_class_labels = sum(
        [d["class_labels"].sum(0) for d in ground_truth_scenes]
    ) / sum([d["class_labels"].shape[0] for d in ground_truth_scenes])
    ...
    print(stats)
    path_to_stats = os.path.join(args.output_directory, f"{args.splits}_stats.npz")

    classes = np.array(dataset.class_labels)
    for c, gt_cp, syn_cp in zip(
        classes, gt_class_labels, syn_class_labels, strict=False
    ):
        print(f"{c}: target: {gt_cp} / synth: {syn_cp}")

    gt_cooccurrences = np.zeros((len(classes) - 2, len(classes) - 2))
    syn_cooccurrences = np.zeros((len(classes) - 2, len(classes) - 2))
    for gt_scene, syn_scene in zip(
        ground_truth_scenes, synthesized_scenes, strict=False
    ):
    ...
    print(f"Saving stats at {path_to_stats}")
    np.savez(
        path_to_stats,
        stats=stats,
        classes=classes,
